...
(define-T-THREAD-LOOP-FUNCTION PEER
  ...
  ((netiorx-thread nil)
   ...
   (FSM-state 'IDLE) ; symbol = [ IDLE | CONNECT | ACTIVE | OPENSENT | OPENCONFIRM | ESTABLISHED ]
   (FSM-timers (FSM-TIMERS-make-default)) ; Timers 'fsm-timers.lisp'
   (FSM-attrib (FSM-ATTRIB-make-default)) ; Configuration/Attributes 'fsm-attrib.lisp'
   (RIB-Adj-in nil)
   (RIB-Adj-out nil)
   ...
   ;; until better way of handling debug streams, set global value in globals.lisp taken as default
   ;; but are shadowed and can be set per thread.
   (*debug-fsm-timers* *debug-fsm-timers*)
   (*debug-fsm-state* *debug-fsm-state*)
   (*debug-fsm-events* *debug-fsm-events*)
   )
  ...
  :loop-entry-block
  ;; check if any FSM timers have been triggered. FSM-TIMERS-poll will enqueue events on %timers-queue
  ((FSM-TIMERS-poll FSM-timers %timers-queue %this-thread-name)
   (PEER-TIMERS-poll PEER-timers %timers-queue %this-thread-name))
  ...
  ((PRINT-ENV2
    ...
    (case FSM-state
      (IDLE	
       (cond ((and RIB-Adj-in RIB-Adj-out router-config peer-config
	      ...
	      (FSM-TIMERS-start-ConnectRetryTimer FSM-timers %this-thread-name)
   ...
   (Event-2-ManualStop
    ...
    (case FSM-state
	 ...
	 (FSM-TIMERS-stop-DelayOpenTimer FSM-timers %this-thread-name)
    ...
    (case FSM-state
      ...
      ((CONNECT ACTIVE)
       ...
       (cond ((FSM-ATTRIB-get-DelayOpen FSM-attrib)
	      ...
	      (FSM-TIMERS-start-DelayOpenTimer FSM-timers %this-thread-name)
